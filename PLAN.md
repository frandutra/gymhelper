...
> Estado actual: **Fase 0 en curso** (0.1–0.4 completos). Marcá `[x]` a medida que cerrás cada slice.
...
- [x] **0.4 — Auth.** Signup, login, logout. Guard en `app/(app)/layout.tsx` + `proxy.ts`. *Aceptación:* registro → ruta protegida; sin sesión redirige. Nota: Supabase pide confirmación de email por defecto — desactivar "Confirm email" en Authentication → Sign In / Providers → Email para que los registros abran sesión directo (igual que EnergyOS).
...
- [ ] **1.1 — Seed del dataset.** `scripts/seed-exercises.ts`: descarga el dataset, valida con Zod (contra el schema del repo), recorta `instructions`/`instruction_steps` a `{es, en}`, upserta por `dataset_id` con service role. Todavía sin media. *Aceptación:* `SELECT count(*) FROM exercises` = 1324; correr el seed dos veces no duplica.
- [ ] **1.2 — Media a Storage.** Extender el seed: crear bucket público `exercise-media`, subir imágenes y GIFs, guardar `image_path`/`gif_path`. Subida reanudable (skip si ya existe). *Aceptación:* un GIF cualquiera carga desde la URL pública del bucket.
