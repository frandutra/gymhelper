...
> Estado actual: **Fase 0 COMPLETA** ✅ (0.1–0.7). **Fase 1 COMPLETA** ✅ (1.1–1.4). **Fase 2 COMPLETA** ✅ (2.1–2.4). **Fase 3 COMPLETA** ✅ (3.1–3.6). **Fase 4 COMPLETA** ✅ (4.1–4.3). **Fase 6 COMPLETA** ✅ (6.1–6.2, correcciones post-feedback). Prod: https://gymhelper-sage.vercel.app. Queda pendiente Fase 5 (backlog, no comprometido). Marcá `[x]` a medida que cerrás cada slice.
...
## Fase 6 — Correcciones y pulido post-feedback

Objetivo: registrar los fixes que salieron de probar la app ya deployada (navegador + celular real), fuera de la secuencia original de slices. **Fase COMPLETA** ✅

- [x] **6.1 — Traducir equipamiento.** El selector de equipamiento en `/exercises` (listado y modo picker de agregar ejercicio) y la ficha de detalle mostraban el valor crudo del dataset en inglés (`"barbell"`, `"dumbbell"`, etc.) mientras que `body_part` ya estaba traducido — reportado por el usuario. Agregados los 28 valores exactos (verificados por query a la base) a `messages/{es,en}.json` bajo `exercises.equipment`; `value={eq}` se mantiene crudo en el `<option>` para que el filtro siga funcionando contra la columna real. *Aceptación:* selector y ficha muestran los 28 valores traducidos; `?equipment=kettlebell` sigue filtrando igual (2 páginas de resultados) — sin errores.
- [x] **6.2 — Rediseño del dashboard.** Reportado por el usuario probando en su celular real: "Hoy no tenés nada prescripto" era un callejón sin salida cuando ningún día coincidía con el weekday de hoy, y la UI se sentía repetitiva (cada día/sesión en su propia caja idéntica). La lista de días pasó a ser siempre visible ("Tus rutinas", no condicional a que "hoy" no tenga match), agrupada por rutina — una card por rutina con sus días como filas divididas adentro (en vez de una card entera repetida por cada día), cada fila con su propio botón "Entrenar hoy" (sigue siendo un tap). "Últimas sesiones" con el mismo tratamiento (una card con filas divididas en vez de 3 cards sueltas). Estado vacío agregado si el usuario todavía no tiene ninguna rutina. De paso, corregida una ambigüedad real de texto: en español "Cerrar sesión" se usaba tanto para logout como para terminar el entrenamiento (mismo string, dos acciones distintas) — el botón de `/workout` ahora dice "Finalizar entrenamiento". *Aceptación:* "Tus rutinas" muestra PPL agrupada con Push y Leg Day adentro, un tap en "Entrenar hoy" sigue llevando directo a `/workout` con la sesión iniciada, "Finalizar entrenamiento" cierra la sesión correctamente — verificado en el navegador, sin errores.
